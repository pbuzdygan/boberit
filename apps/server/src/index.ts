...
import type { AssetFileKind, CreateAssetInput, CreateBinderDocumentInput, TrashKind, IntervalUnit, ScheduleKind, Warranty } from '@boberit/shared';
import { addAssetFile, deleteAssetFile, addDocumentFile, deleteDocumentFile, documentFilesForOcr, setDocumentOcr, getDocumentOcr, saveDocumentOcrText, documentsNeedingOcr, addWebhookDelivery, createHouseholdMember, updateHouseholdMemberRole, removeHouseholdMember, createSession, deleteSession, deleteOtherSessions, userById, updateUserPassword, addAuditEvent, listAuditEvents, storageUsage, createUserAccount, hasUsers, sessionUser, listUserHouseholds, createHouseholdForUser, renameHousehold, setActiveHousehold, listHouseholdMembers, userByLogin, addInboxFile, assignInboxFile, assignInboxToDocument, completeMaintenancePlan, updateMaintenancePlan, deleteMaintenancePlan, deleteInboxFile, createAsset, createDocument, createMaintenancePlan, exportBackupData, getAsset, getAssetFile, getDocumentFile, getInboxFile, listAssets, listDocuments, listStoredFileLocations, listWebhookDeliveries, listWebhooks, createWebhook, updateWebhook, deleteWebhook, webhookTargets, listInboxFiles, listTrash, moveAssetToTrash, moveDocumentToTrash, permanentlyDeleteTrash, permanentlyDeleteHouseholdTrash, restoreBackupData, restoreTrashEntry, saveWarranty, seedDatabase, updateAsset, updateDocument } from './db.js';
...
app.post('/api/v1/assets/:id/files', async (c) => {
  ...
  const file = addAssetFile(scope, assetId, { kind, originalName: upload.originalName, storedName, mimeType: upload.mimeType, byteSize: upload.byteSize });
