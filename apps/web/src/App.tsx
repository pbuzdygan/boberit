...
function DocumentEditDialog({ document, onClose, onSave }: { document: BinderDocument; onClose: () => void; onSave: (id: string, input: Partial<CreateBinderDocumentInput>) => Promise<void> }) {
  ...
  useEffect(()=>{void api.listDocumentTypes().then(r=>setTypes(r.data))},[]);
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); try { await onSave(document.id, { name, typeId:typeId||null, tags: tags.split(",").map((tag) => tag.trim()).filter(Boolean), notes }); onClose(); } finally { setBusy(false); } }
  return <div className="dialog-backdrop" role="presentation"><form className="dialog document-edit-dialog" onSubmit={submit}><header><div><p className="eyebrow">Dokument</p><h2>Edytuj dane</h2></div><button className="icon-button" onClick={onClose} type="button"><Icon label="Zamknij" name="x" /></button></header><div className="document-edit-grid"><label>Nazwa<input required value={name} onChange={(event) => setName(event.target.value)} /></label><label>Typ<select value={typeId} onChange={event=>setTypeId(event.target.value)}><option value="">Brak typu</option>{types.map(type=><option key={type.id} value={type.id}>{type.name}</option>)}</select></label><label className="span-all">Tagi<input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="Oddziel przecinkami" /></label><label className="span-all">Notatka<textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} /></label></div><footer><button className="secondary" onClick={onClose} type="button">Anuluj</button><button className="primary" disabled={busy || !name.trim()} type="submit">{busy ? "Zapisywanie…" : "Zapisz zmiany"}</button></footer></form></div>;
}
...
function ProvisionalFileKind({assetId,fileId,onSave}:{assetId:string;fileId:string;onSave:(assetId:string,fileId:string,kind:AssetFileKind)=>Promise<void>}){const [kind,setKind]=useState<AssetFileKind>('photo');const [busy,setBusy]=useState(false);return <span className="file-kind-picker"><select aria-label="Wybierz typ pliku" value={kind} onChange={e=>setKind(e.target.value as AssetFileKind)}><option value="photo">Zdjęcie</option><option value="receipt">Paragon</option><option value="manual">Instrukcja</option><option value="other">Inne</option></select><button className="secondary" disabled={busy} onClick={()=>{setBusy(true);void onSave(assetId,fileId,kind).finally(()=>setBusy(false))}} type="button">{busy?'Zapisywanie…':'Ustaw typ'}</button></span>}
  ...
  return <>
    ...
    <div className="detail-layout"><div className="detail-main">
      <section className="panel"><header className="panel-title"><div><p className="eyebrow">Dane przedmiotu</p><h2>Informacje</h2></div><button className={`secondary bright save-asset-changes${hasUnsavedChanges ? ' is-pending' : ''}`} disabled={!hasUnsavedChanges || isSaving} onClick={() => void saveChanges()} type="button">{isSaving ? 'Zapisywanie…' : 'Zapisz zmiany'}</button></header><div className="field-grid asset-info-grid">
        <label>Nazwa<input value={name} onChange={(e) => setName(e.target.value)} /></label><label>Producent<input value={manufacturer} onChange={(e) => setManufacturer(e.target.value)} /></label><label>Model<input value={model} onChange={(e) => setModel(e.target.value)} /></label><label>Numer seryjny<input value={serialNumber} onChange={(e) => setSerialNumber(e.target.value)} /></label><label>Kategoria<select value={categoryId} onChange={e=>setCategoryId(e.target.value)}><option value="">Brak</option>{assetOptions.filter(option=>option.kind==='category').map(option=><option key={option.id} value={option.id}>{option.name}</option>)}</select></label><label>Status<select value={itemStatus} onChange={e=>setItemStatus(e.target.value as ItemStatus)}>{(Object.keys(itemStatusMeta) as ItemStatus[]).map(value=><option key={value} value={value}>{itemStatusMeta[value].label}</option>)}</select></label><label>Lokalizacja<select value={locationId} onChange={e=>setLocationId(e.target.value)}><option value="">Brak</option>{assetOptions.filter(option=>option.kind==='location').map(option=><option key={option.id} value={option.id}>{option.name}</option>)}</select></label><label>Link<input value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)} placeholder="https://…" type="url" /></label><label>Data zakupu<input value={purchaseDate} onChange={(e) => changePurchaseDate(e.target.value)} type="date" /></label><label>Ilość<input min="1" value={quantity} onChange={(e) => setQuantity(e.target.value)} type="number" /></label><label>Cena<input min="0" step="0.01" value={price} onChange={(e) => setPrice(e.target.value)} type="number" /></label><label>Tagi<input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="Kuchnia, AGD" /></label><label className="span-all">Notatki<textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Co warto pamiętać?" rows={3} /></label>
      </div></section>
